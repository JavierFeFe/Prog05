 ...
 *Estos son getters que hacen en cierta medida funciones de setters, en este caso los declaro así porque me interesa capturar los posibles errores a la hora de introducir los valores (sobrecarga de métodos que permiten introducir el valor tanto en formato String como en double).*
 ...
 ## Creación de la clase AplicacionCuentaBancaria.java
 
 Desde esta clase generaré el menú necesario para operar con la cuenta, en le primer bucle, despues de pedir el nombre del titular, forzará la introducción de un nº de cuenta válido preguntando despues de cada intento fallido. A continuación se mostará un menú que aparecerá de nuevo despues de realizar cada operación (otro bucle).
 ...
 ````
 *Se crean las variables con las que se trabajará más adelante.
```Java
...
````
*Primer bucle que fuerza que el nº de cuenta bancaria introducido sea válido*
```Java
        boolean dentro = true;
...
````
*Segundo bucle que muestra el menú despues de cada operación (despues de cada operación se ejecutará el método pausa() para dar tiempo a leer el resultado de la operación antes de mostrar nuevamente el menú).*
```Java
...
````
*Un método sencillo que pausa la aplicación (para no tener que escribir el código despues de cada case)*
